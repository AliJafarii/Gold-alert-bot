...
const config = {
  ...
  telegramBotUrl: process.env.TELEGRAM_BOT_URL || 'https://t.me/nbzbazarbot',
  baleBotUrl: process.env.BALE_BOT_URL || 'https://ble.ir/nbzbazarbot',
  telegramPublicChannelId: process.env.TELEGRAM_PUBLIC_CHANNEL_ID || '@nbzbazarch',
  balePublicChannelId: process.env.BALE_PUBLIC_CHANNEL_ID || '',
  channelPublishEnabled: boolEnv('CHANNEL_PUBLISH_ENABLED', false),
  channelPublishIntervalMinutes: numberEnv('CHANNEL_PUBLISH_INTERVAL_MINUTES', 30),
  trialDays: numberEnv('TRIAL_DAYS', 7),
  alertCooldownMinutes: numberEnv('ALERT_COOLDOWN_MINUTES', 30),
  paymentInstructions: process.env.PAYMENT_INSTRUCTIONS || 'برای فعال‌سازی اشتراک، رسید کارت‌به‌کارت یا لینک پرداخت را برای ادمین بفرستید و از دستور /receipt استفاده کنید.',
